...
function DraftField({ field, value, onChange }: { field: string; value: unknown; onChange: (value: unknown) => void }) {
  ...
  if (field === "sections") {
    ...
    return <div className="space-y-3">{sections.map((section, i) => <div className="space-y-2 rounded border p-3" key={section.id}><label className="block text-sm">Section title<Input maxLength={100} value={section.title} onChange={e => update(i, { title: e.target.value })}/></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={!section.hidden} onChange={e => update(i, { hidden: !e.target.checked })}/>Show section</label>{section.entries ? <ProfileCollections partialDates sections={[section.type]} values={{ [section.type]: section.entries }} onChange={values => update(i, { entries: values[section.type] as NonNullable<Resume["sections"]>[number]["entries"] })}/> : section.content !== undefined && <label className="block text-sm">Content<Textarea rows={5} maxLength={20000} value={section.content} onChange={e => update(i, { content: e.target.value })}/></label>}</div>)}</div>;
  }
