...
export default function ResumeAI({ resume, saving, task, setTask, onApplying }: { resume: Resume; saving: boolean; task: Task; setTask: (task: Task) => void; onApplying: (value: boolean) => void }) {
  ...
  return <section id="resume-ai" aria-labelledby="resume-ai-title" className="scroll-mt-6 rounded-xl border border-primary/20 bg-primary/5 p-5 sm:p-6">
    ...
    <label className="my-3 block text-sm"><span className="flex items-center gap-2 font-medium"><Upload className="h-4 w-4"/>Or import an existing resume</span><input className="mt-2 block max-w-full text-sm" type="file" accept=".pdf,.docx,.txt,.md" disabled={busy} onChange={e => { const file = e.target.files?.[0]; if (file) upload.mutate(file); e.target.value = ""; }}/><span className="mt-1 block text-xs text-muted-foreground">PDF, Word or text, up to 3 MB. Scanned PDFs need pasted text.</span></label>
