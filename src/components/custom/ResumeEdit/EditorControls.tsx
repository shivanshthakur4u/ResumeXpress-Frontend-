...
export default function EditorControls({ saving = false, disabled = false }: { saving?: boolean; disabled?: boolean }) {
  ...
  return <details className="rounded-xl border bg-card p-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <label className="text-sm">Resume status<select className="mt-1 block w-full rounded-md border p-2" value={resume.status ?? "draft"} onChange={e => setResumeInfo({ ...resume, status: e.target.value as Resume["status"] })}>{["draft", "ready", "archived"].map(status => <option key={status}>{status}</option>)}</select></label>
      <div className="space-y-2 sm:col-span-2"><label className="block text-sm">Search saved jobs<Input maxLength={100} value={jobSearch} onChange={e => { setJobSearch(e.target.value); setJobPage(1); }}/></label><label className="block text-sm">Target job<select className="mt-1 w-full rounded border p-2" value={resume.targetJob ?? ""} onChange={e => setResumeInfo({ ...resume, targetJob: e.target.value || null })}><option value="">No target job</option>{resume.targetJob && !jobs?.items.some(job => job._id === resume.targetJob) && <option value={resume.targetJob}>Saved target job (outside these results)</option>}{jobs?.items.map(job => <option key={job._id} value={job._id}>{job.title} · {job.company}</option>)}</select></label>{jobsError && <p role="alert" className="text-sm text-red-400">Could not load saved jobs.</p>}<div className="flex items-center gap-2"><Button size="sm" variant="ghost" disabled={jobPage === 1 || jobsLoading} onClick={() => setJobPage(page => page - 1)}>Previous jobs</Button><span className="text-xs">Page {jobPage}</span><Button size="sm" variant="ghost" disabled={jobsLoading || jobPage * 20 >= (jobs?.total ?? 0)} onClick={() => setJobPage(page => page + 1)}>Next jobs</Button></div></div>
      <label className="text-sm">Template<select className="mt-1 block w-full rounded-md border p-2" value={resume.template ?? "legacy"} onChange={e => setResumeInfo({ ...resume, template: e.target.value })}>
        <option value="legacy">Original layout</option>{templates.map(t => <option key={t.id} value={t.id}>{t.name}{t.ats ? " (ATS-safe structure)" : ""}</option>)}
      </select></label>
      <label className="text-sm">Paper<select className="mt-1 block w-full rounded-md border p-2" value={resume.paperSize ?? "A4"} onChange={e => setResumeInfo({ ...resume, paperSize: e.target.value as "A4" | "Letter" })}><option>A4</option><option>Letter</option></select></label>
      <label className="text-sm">Typography<select className="mt-1 block w-full rounded-md border p-2" value={resume.typography ?? "sans"} onChange={e => setResumeInfo({ ...resume, typography: e.target.value as "sans" | "serif" | "mono" })}><option value="sans">Sans serif</option><option value="serif">Serif</option><option value="mono">Monospace</option></select></label>
    ...
    <label className="mt-3 block text-sm">+ Add Section<select value="" className="mt-1 w-full rounded border p-2" disabled={sections.length >= 40} onChange={e => { if (e.target.value) setResumeInfo({ ...resume, sections: [...sections, { id: crypto.randomUUID(), type: e.target.value, title: sectionNames[e.target.value], hidden: false, ...(profileFields[e.target.value] && !["experience", "education", "skills"].includes(e.target.value) ? { entries: [] } : { content: "" }) }] }); }}><option value="">Choose section</option>{Object.entries(sectionNames).map(([key, title]) => <option key={key} value={key}>{title}</option>)}</select></label>
  </details>;
}
